...
export default function CitiesDropdown({
  ...
  // El buscador depende de JS para filtrar: sólo se muestra tras hidratar
  useEffect(() => setMounted(true), [])

  // Close on outside click
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const root = detailsRef.current ?? dropdownRef.current
  ...
  return (
    ...
    >
      <summary className="main-searcher cursor-pointer">
        <p className="searcher text-2xl" style={{ color }}>
          {text}
        </p>
        <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none"
          className="cities-chevron">
      ...
      <div className="cities-panel">
        {/* Search — requiere JS para filtrar */}
        {showSearch && mounted && (
