...
export default function MobileMenu({ items, currentPath, cities }: { items: NavBarLink[], currentPath: string, cities: { city: string; slug: string }[] }) {
                ...
                <nav className="nav-menu-mobile" role="menu">
                    {items.map((item, index) => (
                        item.label.toLowerCase() === "cities" ?
                            <div key={item.href}>
                                <p className={`menu-item text-2xl font-medium text-foreground hover:text-primary transition-all duration-300 hover:scale-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-white ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
                                    }`}>{item.label}</p>
